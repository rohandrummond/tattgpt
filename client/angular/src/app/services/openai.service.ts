...
export class OpenAiService {
  private ideasSubject: BehaviorSubject<Idea[] | null> = new BehaviorSubject<
    Idea[] | null
  >(null);
  public ideasObservable: Observable<Idea[] | null> =
    this.ideasSubject.asObservable();
  ...
  generateIdeas = (formData: any): Promise<boolean> => {
    ...
    return new Promise((resolve, reject) => {
      this.http
        .post<Ideas>('https://localhost:7072/generate-ideas', formData)
        .subscribe({
          next: (response) => {
  ...
  generateImage = (
    ...
    return new Promise((resolve, reject) => {
      this.http
        .post<String>('https://localhost:7072/generate-image', idea)
        .subscribe({
          next: (response) => {
