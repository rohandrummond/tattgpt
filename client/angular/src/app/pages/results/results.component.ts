...
export class ResultsComponent {
    ...
    this.openAi.ideasObservable.subscribe({
      next: (ideas: Idea[] | null) => {
        if (ideas) {
          this.ideaData = ideas;
          if (
            !Object.keys(this.disableSaveButtons).length &&
            !Object.keys(this.disableImageButtons).length
          ) {
            ideas.forEach((idea) => {
  ...
  generateImage = async (idea: Idea): Promise<void> => {
    ...
    if (ideaImg && imgLdr) {
      ...
      const response: boolean | string = await this.openAi.generateImage(
        idea,
        'results'
      );
      ...
      if (!response) {
        this.trackErrors[idea.idea] =
          'Sorry, there was a problem generating your image.';
      }
    }
  };
  ...
  saveIdea = async (idea: Idea): Promise<void> => {
    if (this.userData) {
      if (!this.trackSavedIdeas.hasOwnProperty(idea.idea)) {
        ...
        const response: boolean = await this.supabase.appendImage(
          appendedImage
        );
        if (!response) {
